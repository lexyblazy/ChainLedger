...
func (w *NetworkWorker) start(ctx context.Context, syncBlocks bool) error {
	...
	for {
		...
		if lastKnownHead == 0 || nextBlockNumber > blockDelta {
			...
			bestRpcBlockNumber, err := w.nr.getBestRpcBlockNumber(ctx)
		...
		go func(blockNumber int64) {
			defer wg.Done()
			b, err := w.nr.fetchBlockWithTransactions(ctx, blockNumber)
			if err != nil {
				fetchBlockErr = err
				return
			}
		...
		go func(blockNumber int64) {
			defer wg.Done()
			l, err := w.nr.fetchERC20TransferLogs(ctx, blockNumber)
			if err != nil {
...
func (w *NetworkWorker) fetchTokenMetadata(ctx context.Context, entity db.TokenEntity) (*TokenMetadata, error) {
	...
	decimals, err := w.nr.fetchTokenMetadataDecimals(ctx, tokenAddress)
	if err == nil {
		meta.Decimals = &decimals
	}

	name, err := w.nr.fetchTokenMetadataName(ctx, tokenAddress)
	if err == nil {
		meta.Name = &name
	}
	symbol, err := w.nr.fetchTokenMetadataSymbol(ctx, tokenAddress)
	if err == nil {
		meta.Symbol = &symbol
	}
